hort_qty, 2):,}", delta="Calculated Risk", delta_color="inverse")
...
# FIX: datetime.utcnow() is deprecated in newer Python versions; using timezone-aware or zone subtraction
IST_NOW = datetime.now(timezone.utc if 'timezone' in globals() else None) + timedelta(hours=5, minutes=30)
...
# FIX: We track which tab is active using streamlit session state or a tab selection key
...
try:
    ...
    # FIX: Force columns to upper case to protect against nselib API shifts
...
if trading_mode == "📈 Intraday Cash (Shares)":
    ...
    # FIX: Guarding against division by zero or negative risks
    long_risk = max(long_risk, 0.05) 
    long_qty = min(int(max_risk // long_risk), int(buying_power // long_entry))
    long_qty = max(long_qty, 1) # Minimum 1 share
